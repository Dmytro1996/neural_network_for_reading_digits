...
public class RMSprop implements Optimizer {
    ...
    public void optimize(NetworkEnhanced net, List<INDArray[]> train_data, int epochs, int mini_batch_size, double eta,
        ...
        moveAvg=new INDArray[][]{net.getBiases().stream().map(b->Nd4j.zeros(b.shape()).castTo(DataType.DOUBLE)).toArray(INDArray[]::new),
                    net.getWeights().stream().map(w->Nd4j.zeros(w.shape()).castTo(DataType.DOUBLE)).toArray(INDArray[]::new)};
    ...
    public void update_mini_batch(NetworkEnhanced net, List<INDArray[]> mini_batch, double eta, double lambda, int lenTrainData){
        ...
        for(int i=0;i<net.getNum_layers()-1;i++){
            ...
            //System.out.println("nablas[0][i]:"+nablas[0][i]);
            //System.out.println("nablas[1][i]:"+nablas[1][i]);
            moveAvg[0][i]=moveAvg[0][i].mul(lambda).add(new NDMath().square(nablas[0][i]).mul(1-lambda));
            moveAvg[1][i]=moveAvg[1][i].mul(lambda).add(new NDMath().square(nablas[1][i]).mul(1-lambda));
            //System.out.println("moveAvg[0][i]:"+moveAvg[0][i]);
            //System.out.println("moveAvg[1][i]:"+moveAvg[1][i]);
            net.getBiases().set(i, net.getBiases().get(i).sub(nablas[0][i].div(new NDMath().sqrt(moveAvg[0][i].add(0.000001))).mul(eta)));
            net.getWeights().set(i, net.getWeights().get(i).sub(nablas[1][i].div(new NDMath().sqrt(moveAvg[1][i].add(0.000001))).mul(eta)));
